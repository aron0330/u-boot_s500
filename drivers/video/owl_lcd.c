...
void spi_9bit_wire(__u32 tx)
{
        __u8 i;
//        spi_csx_set(0);
        lcd_deactivate_gpio(&lcd_par.lcd_spi_csx_gpio);
        ...
        for(i=0;i<9;i++)
        {
                LCD_delay_us(1);
//                spi_sck_set(0);
	        lcd_deactivate_gpio(&lcd_par.lcd_spi_sck_gpio);

                LCD_delay_us(1);
                if(tx & (1 << 8))
//                        spi_sdi_set(1);
	        lcd_activate_gpio(&lcd_par.lcd_spi_sdi_gpio);
                else
//                        spi_sdi_set(0);
	        lcd_deactivate_gpio(&lcd_par.lcd_spi_sdi_gpio);
                LCD_delay_us(1);
//                spi_sck_set(1);
	        lcd_activate_gpio(&lcd_par.lcd_spi_sck_gpio);
                LCD_delay_us(1);
                tx <<= 1;
        }
//        spi_sck_set(0);
	        ...
	        LCD_delay_us(1);
//        spi_csx_set(1);
	        lcd_activate_gpio(&lcd_par.lcd_spi_csx_gpio);
	        LCD_delay_us(1);
}

//add by bill, init cubiescreen, end
...
//add by bill, init cubiescreen
        tmp |= gpio_bit_to_pwr(&lcd_par.lcd_spi_csx_gpio);
        tmp |= gpio_bit_to_pwr(&lcd_par.lcd_spi_sck_gpio);
        tmp |= gpio_bit_to_pwr(&lcd_par.lcd_spi_sdi_gpio);
//add by bill, init cubiescreen, end
	if(tmp){		
...
static int fdtdec_disable_lcd(void)
{
	...
	lcd_deactivate_gpio(&lcd_par.lcdreset_gpio);	//add by bill, init cubiescreen
...
//add by bill, init cubiescreen
        ...
        owl_fdtdec_decode_gpio(
                gd->fdt_blob, dev_node, "lcd_spi_csx",
                        &par->lcd_spi_csx_gpio);

        owl_fdtdec_decode_gpio(
                gd->fdt_blob, dev_node, "lcd_spi_sck",
                        &par->lcd_spi_sck_gpio);
                         
        owl_fdtdec_decode_gpio(
                gd->fdt_blob, dev_node, "lcd_spi_sdi",
                        &par->lcd_spi_sdi_gpio);
